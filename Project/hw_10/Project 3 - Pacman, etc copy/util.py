...
class Counter(dict):
    ...
    def argMax(self):
        ...
        if len(list(self)) == 0: return None
        # all = list(self.items())
        # values = [x[1] for x in all]
        # maxIndex = values.index(max(values))
        # return random.choice(all[maxIndex]) #[0]
        maxValue = max(self.values())
        return random.choice([key for (key, value) in self.items() if value == maxValue])
    ...
    def round(self, p):
        """
        Creates and returns a new Counter with the values  
        of this one rounded to p places. 

        Useful for printing weights and features when
        only a few decimal digits matter.
        """
        return {a:round(v, p) for (a,v) in self.items()}
